...
fs.writeFileSync(process.env.USERPROFILE + "\\Desktop\\App_backup_before_inventory_fix_v2.jsx", text, "utf8");
...
function replaceFunction(source, functionName, replacement) {
  ...
  if (start === -1) throw new Error("Cannot find function: " + functionName);
...
if (start === -1 || end === -1 || end <= start) {
  throw new Error("Cannot find Search & Filter to Transaction History inventory block.");
}

const newInventoryBlock = `
                {/* Search & Filter */}
                <div style={{ background:'white', border:'1px solid #eee', borderRadius:'16px', padding:'14px', marginBottom:'16px', boxShadow:'0 2px 10px rgba(0,0,0,0.04)' }}>
                  <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'2fr 1fr 1fr 1fr', gap:'10px', alignItems:'end' }}>
                    ...
                    <div style={{ background:'#fff8dc', border:'1px solid #fdd412', borderRadius:'12px', padding:'10px' }}>
                      <p style={{ margin:'0 0 4px', fontSize:'11px', color:'#777', fontWeight:'800' }}>Total Items</p>
                      <p style={{ margin:0, fontSize:'20px', fontWeight:'900', color:'#ca1b1b' }}>{inventoryItems.length}</p>
                    </div>
                    <div style={{ background:'#f0fff4', border:'1px solid #c8e6c9', borderRadius:'12px', padding:'10px' }}>
                      ...
                      <p style={{ margin:0, fontSize:'20px', fontWeight:'900', color:'#2d8a4e' }}>
                        {inventoryItems.filter(i=>Number(i.current_stock||0)<=Number(i.min_stock||0)&&Number(i.min_stock||0)>0).length}
                      </p>
                    </div>
                  </div>
                </div>

                {/* Clean Categorized Inventory Table */}
                                  ...
                                  return (
                                    <tr key={item.id} style={{ borderTop:'1px solid #eee', background:isLow?'#fff5f5':'white' }}>
                                      ...
                                      <td style={{ padding:'10px', textAlign:'center', verticalAlign:'top' }}>
                                        {isEditing ? (
                                          <input value={editItemFields.unit ?? item.unit} onChange={e=>setEditItemFields(p=>({...p,unit:e.target.value}))} style={{ ...inputStyle, marginBottom:0, textAlign:'center' }} />
                                        ) : item.unit}
                                      </td>
...
text = text.slice(0, start) + newInventoryBlock + text.slice(end);
