...
const backup = `src/App.jsx.backup-before-inventory-category-price-print-v2-${stamp}`
...
if (!src.includes('function printInventoryCategoryPriceList')) {
  insertAfterAsyncFunction('saveInventoryItemEdit', `
...
if (!src.includes('PRINT CATEGORY PRICE LIST')) {
  const valueNeedle = 'value={inventoryCategoryFilter}'
  const valueAt = src.indexOf(valueNeedle)
  if (valueAt < 0) throw new Error('inventoryCategoryFilter select not found')

  const selectClose = src.indexOf('</select>', valueAt)
  if (selectClose < 0) throw new Error('inventoryCategoryFilter closing select not found')

  const insertAt = selectClose + '</select>'.length
 ...
 >
  ...
  src = src.slice(0, insertAt) + button + src.slice(insertAt)
  changes++
