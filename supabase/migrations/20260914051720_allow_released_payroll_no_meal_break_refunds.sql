-- Allow released-payroll No Meal Break corrections when historical approved
-- OT/UT records are already represented in the released payroll snapshot.
-- Rollback: re-apply the previous resolve_time_adjustment_to_next_cutoff
-- function body from migration 20260901153441.
...
'Atomically resolves a pending released-period OT/UT/No Meal Break request, preserves the released payroll snapshot, allows historical released-payroll OT/UT evidence, and creates at most one linked adjustment in a later uncomputed cutoff.';
