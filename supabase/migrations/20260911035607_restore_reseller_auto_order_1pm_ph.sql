-- Restore reseller automatic ordering to 1:00 PM Asia/Manila.
-- Manual ordering already uses the same 1:00 PM deadline and is intentionally unchanged.
-- This migration reverses only the prior 10:00 AM automatic-order cutoff release.

alter table public.reseller_auto_order_schedules
  alter column submission_time set default time '13:00';

update public.reseller_auto_order_schedules
set submission_time = time '13:00',
    updated_at = now()
where submission_time is distinct from time '13:00';

-- Preserve the current production function bodies while changing only the
-- automatic-order cutoff and its user-facing wording back to 1:00 PM.
...
begin
  ...
  loop
    ...
    new_definition := replace(old_definition, '10:00 AM', '1:00 PM');
    new_definition := replace(new_definition, '10:00', '13:00');
...
-- pg_cron runs in UTC: 05:00 UTC = 13:00 Asia/Manila.
do $auto_order_cron$
declare
  existing_job record;
...
-- Fail fast if the automatic-order functions or the manual-order write guard
-- do not agree on the restored 1:00 PM Philippine-time rule.
do $auto_order_verify$
declare
  remaining_10am integer;
begin
  select count(*)
  into remaining_10am
  from pg_proc p
  ...
  if remaining_10am <> 0 then
    raise exception 'Automatic-order cutoff restore incomplete: % function(s) still reference 10:00 AM.', remaining_10am;
  end if;
