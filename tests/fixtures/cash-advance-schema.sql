...
create table public.employees (id uuid primary key,employee_code text,full_name text,is_active boolean default true,daily_rate numeric,sil_balance integer default 0,sick_leave_balance integer default 0,vacation_leave_balance integer default 0);
create table public.final_pay_records (id integer generated always as identity primary key,employee_id uuid,employee_name text,employee_code text,separation_reason text,last_working_date text,last_salary numeric,pro_rated_13th numeric,sil_pay numeric,separation_pay numeric,cash_advance_deduction numeric,total_final_pay numeric,created_at timestamptz default now());
