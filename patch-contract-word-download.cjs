...
const backup = `src/App.jsx.backup-before-contract-word-download-${stamp}`;

let src = fs.readFileSync(path, 'utf8');
fs.copyFileSync(path, backup);

const start = src.indexOf('  function printContractSummary(c) {');
const end = src.indexOf('  function getRegularizationDueDate(emp) {', start);

if (start < 0 || end < 0) {
  throw new Error('Could not find printContractSummary function boundaries.');
}

const newFunction = `  function printContractSummary(c) {
    ...
    const statusRaw = cleanText(c?.status || 'active').toLowerCase();
    const statusLabel = titleCase(statusRaw || 'active');
    ...
    const preparedBy = cleanText(currentAdminLabel || 'Admin');
...
src = src.slice(0, start) + newFunction + src.slice(end);
fs.writeFileSync(path, src, 'utf8');

console.log('DONE: Contract print now downloads a Word-compatible .doc file.');
