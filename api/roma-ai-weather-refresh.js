...
export default async function handler(req,res){
  if(req.method==='GET')return json(res,200,{ok:true,service:'roma-ai-weather-refresh',version:'2026.08.13.1-weather',edge:WEATHER_EDGE})
  ...
  try{
    ...
    if(!response.ok){
      return json(res,200,{
        ...
        message:payload?.details||payload?.live_error||'Live PAGASA refresh failed; Roma AI may use the last verified cache.'
      })
    }
    ...
    return json(res,200,{
      ...
      provider:payload?.provider||'DOST-PAGASA',
      accessGateway:payload?.access_gateway||WEATHER_EDGE
    })
  }catch(error){
    console.error('Roma AI weather refresh error',{status:error.status,message:error.message})
    return json(res,200,{ok:false,refreshed:false,error:error.message||'weather_refresh_failed'})
  }
}
